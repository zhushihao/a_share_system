...
class DataSourceResilience:
    ...
    def fetch_kline(self, code: str, start_date: str, end_date: str,
        ...
        if self._try_source("mootdx_offline", code):
            try:
                df = self._fetch_mootdx_offline_kline(code, start_date, end_date, period)
        ...
        if self._try_source("mootdx_realtime", code):
            try:
                df = self._fetch_mootdx_realtime_kline(code, start_date, end_date, period)
                if df is not None and len(df) > 0:
                    result.data = df
                    result.success = True
    ...
    def _fetch_mootdx_offline_kline(self, code: str, start: str, end: str, period: str) -> Optional[pd.DataFrame]:
        """调用 mootdx 离线（通达信本地数据）获取K线"""
        provider = self._get_mootdx()
        if provider is None:
            return None
        try:
            return provider.fetch_kline(code, start, end, period, source="offline")
        except Exception as e:
            self._obs.log("WARN", f"mootdx offline failed for {code}: {str(e)}", "DataSourceResilience")
            return None
    
    def _fetch_mootdx_realtime_kline(self, code: str, start: str, end: str, period: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            return provider.fetch_kline(code, start, end, period, source="realtime")
        except Exception as e:
    ...
    def _fetch_ifind_kline(self, code: str, start: str, end: str, adjust: str) -> Optional[pd.DataFrame]:
        ...
        def _call():
            return ifind_daily_kline(code, start, end, adjust=adjust)
    ...
    def _fetch_sfd_kline(self, code: str, start: str, end: str, adjust: str) -> Optional[pd.DataFrame]:
        ...
        def _call():
            return _sfd_get_price(ticker, start, end, interval="D", adjust=adjust)
